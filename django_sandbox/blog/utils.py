...
class PostHelper:
    ...
    def like_post(post, user, liked):
        ...
        If there is an existing active Like make it inactive for unliking otherwise create a new Like object.
        (inactive likes don't count - they can keep count of number of times a post was unliked by the user, idk if
        it'll be useful or not but lets hope it will otherwise we'll remove it)
        """
        ...
        if liked:
            ...
            if not existing_like_obj:
                raise ValidationError("No Existing like for this post! Can't unlike")
